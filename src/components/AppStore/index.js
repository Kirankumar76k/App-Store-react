...
class AppStore extends Component {
  state = {searchItem: '', activeTabId: tabsList[0].tabId}
  ...
  render() {
    const {searchItem, activeTabId} = this.state
    const filterTabList = this.getFilterAppList()

    const filterBySearchItem = filterTabList.filter(eachItem =>
          ...
          <ul className="tabItem-container">
            {tabsList.map(eachItem => (
              <TabItem
                key={eachItem.tabId}
                tabDetails={eachItem}
                onFilterTabItems={this.onFilterTabItems}
                isActive={eachItem.tabId === activeTabId}
              />
            ))}
          </ul>
          <ul className="AppItem-container">
            {filterBySearchItem.map(eachItem => (
